...
const KITCHEN_PASSWORD = 'masala2026';

type KitchenGateProps = {
  children: ReactNode;
  onExit: () => void;
};

export default function KitchenGate({ children, onExit }: KitchenGateProps) {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem('kitchen-unlocked');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (password === KITCHEN_PASSWORD) {
      setUnlocked(true);
      sessionStorage.setItem('kitchen-unlocked', 'true');
      setError(false);
    } else {
      setError(true);
      setAttempts((a) => a + 1);
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-3xl border border-line bg-surface p-8 shadow-xl">
          ...
          <form onSubmit={handleSubmit}>
            <label className="mb-2 block text-xs font-bold text-ink">Password</label>
